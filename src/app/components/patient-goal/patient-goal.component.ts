...
import { PatientGoal } from './PatientGoal.model';
...
@Component({
  ...
  styleUrl: './patient-goal.component.css'
})
export class PatientGoalComponent {
  ...
  constructor(
    ...
    private PatientGoalService: PatientService
  ) {}
  ...
  loadPatientEncounters(id: string) {
    this.PatientGoalService.fetchPatientEncounters(id).subscribe({
      next: (data) => {
        this.PatientGoals = data;
        console.log('✅ Patient Encounters:', this.PatientGoals);
      },
      error: (error) => {
