...
import { PatientImmunization } from './PatientImmunization.model';
...
@Component({
  ...
  styleUrl: './patient-immunization.component.css'
})
export class PatientImmunizationComponent {
  ...
  constructor(
    ...
    private patientImmunizationService: PatientService
  ) {}
  ...
  loadPatientEncounters(id: string) {
    this.patientImmunizationService.fetchPatientEncounters(id).subscribe({
      next: (data) => {
        this.patientImmunizations = data;
        console.log('✅ Patient Encounters:', this.patientImmunizations);
      },
      error: (error) => {
