...
import { PatientCareTeam } from './PatientCareTeam.model';
...
@Component({
  ...
  styleUrl: './patient-care-team.component.css'
})
export class PatientCareTeamComponent {
  ...
  loadPatientEncounters(id: string) {
    this.patientEncounterService.fetchPatientEncounters(id).subscribe({
      next: (data) => {
        this.patientCareTeam = data;
        console.log('✅ Patient Encounters:', this.patientCareTeam);
      },
      error: (error) => {
